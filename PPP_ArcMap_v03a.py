...
def check_and_write_owner_index_selection():
	"""
	Check if exactly one polygon is selected in the "OwnerIndex" layer.
	If so, write the objectid and pid to PIDOID.json file.
	Returns True if OwnerIndex polygon was processed, False otherwise.
	"""
	try:
		...
		output_file = 'C:/Users/Public/Public Mapfiles/M1_Files/PIDOID.json'
		
		# Find OwnerIndex layer
		mxd = arcpy.mapping.MapDocument("CURRENT")
		layers = arcpy.mapping.ListLayers(mxd)
		
		owner_index_layer = None
		for layer in layers:
			if layer.name == "OwnerIndex":
				owner_index_layer = layer
				break
		
		if not owner_index_layer:
			arcpy.AddMessage('No OwnerIndex layer found, checking for selected parcels...')
			del mxd
			return False
		
		# Check selection count
		try:
			temp_layer_name = "temp_owner_index_check"
			arcpy.MakeFeatureLayer_management(owner_index_layer.name, temp_layer_name)
			count = int(arcpy.GetCount_management(temp_layer_name).getOutput(0))
			
			if count == 1:
				arcpy.AddMessage('Found 1 selected polygon in OwnerIndex layer')
				
				# Get the objectid and pid from the selected polygon
				for row in arcpy.SearchCursor(temp_layer_name):
					objectid = row.getValue("OBJECTID") if hasattr(row, "OBJECTID") else row.getValue("objectid")
					pid = row.getValue("PID") if hasattr(row, "PID") else row.getValue("pid")
					...
					arcpy.AddMessage('OwnerIndex selection written to PIDOID.json')
					arcpy.AddMessage('ObjectID: {0}, PID: {1}'.format(objectid, pid))
					
					# Clean up and return True to indicate OwnerIndex was processed
					arcpy.Delete_management(temp_layer_name)
					del mxd
					return True
			
			elif count > 1:
				arcpy.AddMessage('Multiple polygons selected in OwnerIndex layer ({0}), checking for selected parcels...'.format(count))
			else:
				arcpy.AddMessage('No polygons selected in OwnerIndex layer, checking for selected parcels...')
			
			# Clean up temp layer
			arcpy.Delete_management(temp_layer_name)
			
		except Exception as e:
			arcpy.AddMessage('Error processing OwnerIndex layer: {0}'.format(str(e)))
		...
		arcpy.AddError('Error checking OwnerIndex layer: {0}'.format(str(e)))
		return False
...
def write_parcel_owner_info_json():
	...
	'C:/Users/Public/Public Mapfiles/M1_Files/ArcMakePIDFromParcelOwnerInfo.json' in the specified JSON format.
	"""
...
def main():
	"""
	Main function to execute the parcel owner information extraction process.
	"""
	...
	# OWNERINDEX LAYER ##################################################
	# First, check if exactly one polygon is selected in OwnerIndex layer
	if check_and_write_owner_index_selection():
		arcpy.AddMessage('OwnerIndex polygon processed successfully!')
		arcpy.AddMessage('Launching Power PID Producer Python script...')
		
		# Launch the Python script
		pypath = 'C:/"Program Files"/Python312/python.exe'
		py_script = 'F:/Research Department/Code/RP3/PPP_Ownership_to_Sale_v01.py'
	...
	# PARCEL LAYER ######################################################
	# If no OwnerIndex selection, proceed with parcel processing
	# Find and create parcel layer from selected parcels
	if not find_and_create_parcel_layer():
		arcpy.AddError('No OwnerIndex polygon selected and no parcel layers with selected features found.')
		arcpy.AddError('Please select either 1 polygon in OwnerIndex layer or parcels in a parcel layer before running this script.')
		return
	...
	arcpy.AddMessage('Launching Power PID Producer Python script...')
	pypath = 'C:/"Program Files"/Python312/python.exe'
	py_script = 'F:/Research Department/Code/RP3/PPP_Parcels_to_Ownership_v01.py'
