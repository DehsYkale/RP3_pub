...
def write_parcel_owner_info_json():
	...
	try:
		...
		# FIXED: Use arcpy.da.SearchCursor instead of arcpy.SearchCursor
		# and explicitly include SHAPE@ token to access geometry
		field_list = []
		
		# Get all field names from the layer
		for field in desc.fields:
			if field.type != 'Geometry':  # Skip geometry fields, we'll use SHAPE@ token
				field_list.append(field.name)
		
		# Add SHAPE@ token to access geometry
		field_list.append('SHAPE@')
		
		# Process each selected parcel using the new cursor
		with arcpy.da.SearchCursor("parcel_lyr", field_list) as cursor:
			for row in cursor:
				index += 1
				
				# Create a dictionary for easy field access
				row_dict = {}
				for i, field_name in enumerate(field_list[:-1]):  # Exclude SHAPE@ from field mapping
					row_dict[field_name.lower()] = row[i]
				
				# Get geometry (last item in the row)
				geometry = row[-1]
				
				# Get parcel centroid coordinates
				lon, lat = get_parcel_centroid(geometry)
				
				# Process APN information
				apn = ""
				if 'apn' in row_dict and row_dict['apn'] and str(row_dict['apn']).upper() != 'NONE':
					apn = str(row_dict['apn'])
				elif 'altapn' in row_dict and row_dict['altapn'] and str(row_dict['altapn']).upper() != 'NONE':
					apn = str(row_dict['altapn'])
				
				# Get propertyid
				propertyid = ""
				if 'propertyid' in row_dict and row_dict['propertyid']:
					propertyid = str(row_dict['propertyid'])
				...
				try:
					...
					if mail_address_field and 'mailaddress' in row_dict and row_dict['mailaddress']:
						address_lines = str(row_dict['mailaddress']).split('\n')
						if len(address_lines) > 0:
							mailstreet = address_lines[0]
					elif 'mailstreet' in row_dict and row_dict['mailstreet']:
						mailstreet = str(row_dict['mailstreet'])
					
					if 'mailcity' in row_dict and row_dict['mailcity']:
						mailcity = str(row_dict['mailcity'])
					if 'mailstate' in row_dict and row_dict['mailstate']:
						mailstate = str(row_dict['mailstate'])
					if 'mailzip' in row_dict and row_dict['mailzip']:
						mailzip = str(row_dict['mailzip'])
				...
				zoning = ""
				if 'zoning' in row_dict and row_dict['zoning']:
					zoning = str(row_dict['zoning'])
				
				# Get acres
				acres = 0.0
				if 'acres' in row_dict and row_dict['acres']:
					try:
						acres = float(row_dict['acres'])
						total_acres += acres
