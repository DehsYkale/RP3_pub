...
def change_empty_to_zero(dData, fin):
	for row in dData:
		if 'BenchmarkObservedClosings' in fin:
			# if dData[row]['Annual Starts'] == '':
			# 	dData[row]['Annual Starts'] = 0
			if dData[row]['ObsClosings_TotalAnnual'] == '':
				dData[row]['ObsClosings_TotalAnnual'] = 0
...
# Make dict of total Starts and Closings for each builder
def make_dict_of_homebuilder_closings(dHB_closings):
	# Create dict to aggregate builder metrics
	dHB = {}

	for row in dHB_closings:
		# Skip Total row
		if dHB_closings[row]['Builder'] == 'Total':
			continue
		# Builder not in dict, add it
		elif dHB_closings[row]['Builder'] not in dHB:
			no_closings = dHB_closings[row]['ObsClosings_TotalAnnual'].replace(',', '')
			dHB[dHB_closings[row]['Builder']] = {'Closings': int(no_closings)}
		# Builder in dict, add to it
		else:
			no_closings = dHB_closings[row]['ObsClosings_TotalAnnual'].replace(',', '')
			dHB[dHB_closings[row]['Builder']]['Closings'] += int(no_closings)
...
td.banner('Zonda Enterprize Buldr Sub Arcmap List Maker v02')
...
hb_closings_table_name = f'BenchmarkObservedClosings {year_qtr}.csv'
...
for market in lMarkets:
	...
	# Change '' to 0 in Starts and Closings
	dSubdivisions = change_empty_to_zero(dSubdivisions, fin)
...
for market in lMarkets:
	...
	fin_hb_close = f'{zonda_path}{market} {hb_closings_table_name}'
	
	# Check if the file exists
	if not os.path.exists(fin_hb_close):
		td.warningMsg(f' Zonda HB Closings file for {market} does not exist!')
		ui = td.uInput('\n Continue [00]... > ')
		if ui == '00':
			exit('\n Terminating program...')
		continue

	# Make dict of homebuilder closings
	...
	# Make dict of total Starts and Closings for each builder
	dHB = make_dict_of_homebuilder_closings(dHB_closings)

	# if market == 'HOU':
	# 	with open('C:/TEMP/hb1.csv', 'w', newline='') as f:
	# 		fout = csv.writer(f)
	# 		fout.writerow(['Builder', 'Annual Closings'])
	# 		for row in dHB:
	# 			fout.writerow([row, dHB[row]['Closings']])
	# 	lao.openFile('C:/TEMP/hb1.csv')
	# 	ui = td.uInput('\n Continue [00]... > ')
	# 	if ui == '00':
	# 		exit('\n Terminating program...')
