...
def make_Raw_Comps_CSV():
	csvRawCompsFileName = '{0}TUC RED News {1}.csv'.format(lao.getPath('comps'), td.today_date())
	if exists(csvRawCompsFileName):
		...
		with open(csvRawCompsFileName, 'w', newline='') as f:
			fout = csv.writer(f)
			header = 'Seller Entity', 'Seller Person', 'Seller Street', 'Seller City', 'Seller State', 'Seller Zip', 'Seller Phone', 'Buyer Entity', 'Buyer Person', 'Buyer Street', 'Buyer City', 'Buyer State', 'Buyer Zip', 'Buyer Phone', 'Sale Date', 'Type', 'Lead Parcel', 'Sale Price', 'Zoning', 'Longitude', 'Parcels', 'Rec Doc', 'Latitude', 'City', 'Subdivision', 'Source ID', 'Acres', 'Lots', 'Source', 'State', 'County'
...
def formatSellerBuyer(aList, sellerbuyer):
	while 1:
		dataList = []
		lao.banner('Tucson RED Scrape v01')
		zipEntered = False
		count = -1
		# print aList
		# print len(aList)
...
td.banner('Tucson RED Scrape v01')
...
while 1:
	...
	dataList.extend(['RED News', 'Arizona', 'Pima'])
