...
def check_if_files_current():
	...
	while is_recent is False:
		...
		is_recent = aws.was_file_modified_recently(file_to_check='Make PID from Parcels')
		if is_recent:
			update_with_parcels_or_PID = 'Parcels'
			return update_with_parcels_or_PID
		is_recent = aws.was_file_modified_recently(file_to_check='Zoom to Polygon')
		if is_recent:
			update_with_parcels_or_PID = 'PID'
			return update_with_parcels_or_PID
		td.warningMsg('\n Selected parcels or PID are not current.\n)' \
		print' Click Save Parcels in M1 or enter PID in GUI and Submit to continue.')
		ui = td.uInput('\n Continue [00] > ')
		if ui == '00':
			exit('\n Terminating program...')
...
# Check if the files were made recently
update_with_parcels_or_PID = check_if_files_current()
