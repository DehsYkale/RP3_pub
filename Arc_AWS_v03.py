...
def queryFeatures(queryLyr, fieldName, fieldValueList):
	...
	# query the features
	# Try numeric format first (without quotes), fallback to text format if it fails
	try:
		# For numeric fields
		query_result = queryLyr.query(where="{} IN ({})".format(fieldName, ",".join(fieldValueList)), return_extent_only=False)
		print('\n   Using numeric query format')
	except Exception as e:
		print('\n   Numeric query failed, trying text format...')
		print('   Error: {0}'.format(str(e)))
		try:
			# For text fields
			query_result = queryLyr.query(where="{} IN ('{}')".format(fieldName, "','".join(fieldValueList)), return_extent_only=False)
			print('\n   Using text query format')
		except Exception as e2:
			print('\n   Both query formats failed!')
			print('   Numeric error: {0}'.format(str(e)))
			print('   Text error: {0}'.format(str(e2)))
			raise e2
